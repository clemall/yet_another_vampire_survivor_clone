...
fn spawn_claw_attack(
    ...
    mut claw_spawner: Query<&mut AttackTimer, With<ClawSpawner>>,
    ...
    if let Ok(mut attack_timer) = claw_spawner.get_single_mut(){
...
fn claw_attack_animation_and_collider(
    mut claws: Query<(&mut Transform, &AttackDuration), With<Claw>>,
) {
...
fn claw_attack_despawn(
    ...
    for (entity, attack_duration)  in &mut claws {
...
fn claw_damage(
    ...
    for (collider, transform, claw, mut seen_enemies) in &mut claws {
